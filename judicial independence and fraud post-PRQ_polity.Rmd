...
#####Loading dataset####
vdem.nodems <- read.csv("./vdem-2018-no-dems-polcomp-post1945-polity-oct2020-condensed-tidy.csv") #For broader sample
...
vdem.nodems.elections <- vdem.nodems.elections %>% filter(e_polcomp >= 0)
...
```{r histogram}
p.hist <- ggplot(vdem.nodems.elections, aes(polcomp.lag, fill = factor(reform_positive.lag))) + geom_histogram(bins = 18) + 
                   guides(fill = guide_legend(title = "Treatment")) + labs(x = "Political competition", y = "Count") + scale_fill_grey() + theme_bw()
...
```{r combining me plots}
p.combined.ebal <- cowplot::plot_grid(p2.elcomp, p2.legov, p2.polity, p2.uds, align = "hv", nrow = 2, label_size = 12, hjust = 0)
p.combined.ebal
...
```{r}
#Entropy balanced
stargazer(mm.elirreg.posreform.elcomp.all, mm.elirreg.posreform.oversight, mm.elirreg.posreform.polity.all,
          mm.elirreg.posreform.uds.all,
          digits = 2, omit = c("COW.factor", "COWcode", "year.factor", "year"),
