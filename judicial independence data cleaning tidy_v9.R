...
vdem <- read.csv("C:/Users/Cole/Documents/Research topics literature/V-Dem/Country_Year_V-Dem_Extended_CSV_v8/V-Dem-CY+Others-v8.csv")
...
vdem.small <- vdem.short %>% dplyr::select(COWcode, year, transitional, v2elvotbuy.inv, v2elirreg.inv, v2elintim.inv, v2elpeace.inv, v2juncind, 
                                    ...
                                    v2x_freexp, v2xme_altinf, e_polcomp, e_parcomp, e_exconst, e_democ, e_autoc, 
                                    ...
                                    v2mecrit, v2mebias, e_parreg, v2svdomaut, v2stfisccap_ord)
...
write.csv(vdem.small, "C:/Users/Cole/Documents/Research projects/Judicial independence and fraud/vdem-small-v8-post1943-tidy.csv")

###Reading in vdem.small
vdem.small <- read.csv("C:/Users/Cole/Documents/Research projects/Judicial independence and fraud/vdem-small-v8-post1943-tidy.csv")
...
vdem.small <- vdem.small %>% group_by(COWcode) %>% mutate(domest.autonomy.lag = lag(v2svdomaut, 1))
vdem.small <- vdem.small %>% group_by(COWcode) %>% mutate(state.revsource.lag = lag(v2stfisccap_ord, 1))
...
vdem.small <- vdem.small %>% group_by(COWcode) %>% mutate(domest.autonomy.2lag = lag(v2svdomaut, 2))
vdem.small <- vdem.small %>% group_by(COWcode) %>% mutate(state.revsource.2lag = lag(v2stfisccap_ord, 2))
...
write.csv(vdem.small, "./vdem-small-v8-post1943-tidy.csv")
...
###Writing vdem.nodems
write.csv(vdem.nodems, "./vdem-2018-no-dems-post1945-polity-v8-condensed-tidy.csv")
